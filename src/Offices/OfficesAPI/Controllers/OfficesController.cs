...
namespace OfficesAPI.Controllers
{
    ...
    public class OfficesController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CreateOfficeDto request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);

            var createdOffice = await _officeService.CreateOfficeAsync(request);
            return CreatedAtAction(nameof(GetById), new { id = createdOffice.Id }, createdOffice);
        }

        // US-30 (AC-3): Редактирование существующего офиса
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(string id, [FromBody] UpdateOfficeDto request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
        ...
        [HttpPatch("{id}/status")]
        public async Task<IActionResult> ChangeStatus(string id, [FromBody] ChangeOfficeStatusDto request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);

            var result = await _officeService.ChangeStatusAsync(id, request);
