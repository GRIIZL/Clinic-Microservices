...
namespace ProfilesAPI.Controllers
{
    ...
    public class DoctorsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDoctorsForPatients([FromQuery] DoctorQueryParametersDto query)
        {
            var doctors = await _doctorService.GetDoctorsForPatientsAsync(query);
...
// Эндпоинт для администраторов и персонала (US-22, US-24, US-26, US-28)
        [HttpGet("admin-view")]
        public async Task<IActionResult> GetDoctorsForAdmin([FromQuery] DoctorQueryParametersDto query)
        {
            var doctors = await _doctorService.GetDoctorsForAdminAsync(query);
            if (!doctors.Any())
            {
        ...
        [HttpPatch("{id}/status")]
        public async Task<IActionResult> ChangeStatus(Guid id, [FromBody] ChangeDoctorStatusDto request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);

            var result = await _doctorService.ChangeDoctorStatusAsync(id, request);
        ...
        [HttpPost]
        public async Task<IActionResult> CreateDoctor([FromBody] CreateDoctorProfileDto request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);

            var result = await _doctorService.CreateDoctorProfileByReceptionistAsync(request);
            if (result == null)
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateDoctorProfile(Guid id, [FromBody] UpdateDoctorProfileDto request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);

            var result = await _doctorService.UpdateDoctorProfileAsync(id, request);
