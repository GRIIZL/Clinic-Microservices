...
namespace ProfilesAPI.Controllers
{
    ...
    public class PatientsController : ControllerBase
    {
        ...
        {
            var result = await _patientService.GetPatientsListAsync(name);
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CreatePatientProfileDto request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
            
            var result = await _patientService.CreatePatientProfileAsync(request);
            return Ok(result);
        }

        // Эндпоинт клика по кнопке "Yes, it's me" для связывания (AC-7)
        [HttpPost("{id}/link-account")]
        public async Task<IActionResult> LinkAccount(Guid id, [FromQuery] Guid accountId)
        {
            var success = await _patientService.LinkAccountToExistingProfileAsync(id, accountId);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] CreatePatientProfileDto request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
        ...
        {
            var success = await _patientService.DeletePatientProfileAsync(id);
